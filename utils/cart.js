 ...
 * @param {购物车类型名称} cartTypeName 
 */
function getShopCartGoods(goodsId, specSku, cartTypeName) {
  let cart = null
  let carts = getShopCart(cartTypeName)
  if (carts == null) return cart;
  //循环获取数据
  carts.forEach((item) => {
    if (item.goodsId == goodsId && item.specSku == specSku) {
      cart = item
      return cart
    }
  })
 ...
 * @param {购物车类型} cartTypeName 
 */
function shopCartIndex(goodsId,specSku, carts) {
  let count = carts.length;
  if (count == 0) return -1
  for (let i = 0; i < count; i++) {
    if (carts[i].goodsId == goodsId && carts[i].specSku == specSku) {
      return i
    }
  }
  return -1;
...
function addShopCart(goods, cartTypeName, isGift) {
  ...
  //获取购物车数据
  let cart = getShopCartGoods(goods.id, goods.specSku, cartTypeName)
  ...
  if (cart == null) {
    ...
    //sku
    if (goods.specSku) {
      cart.specSku = goods.specSku
    } else {
      cart.specSku = ''
    }
    if (goods.specDesc) {
      cart.specDesc = goods.specDesc
    } else {
      cart.specDesc = ''
    }
    if(goods.tradeTypeId){
      cart.tradeTypeId = goods.tradeTypeId
    }else {
      cart.tradeTypeId = 3
    }
    if(goods.limitDate){
      cart.limitDate = goods.limitDate
    }else {
      cart.limitDate = ''
    }
    cart.self = goods.self
    //购物车页面 是否选中，默认选中
    cart.isChoosed = true
    //放入缓存
    ...
    //商品已购数量
    goods.buyCount = cart.total
    let index = shopCartIndex(goods.id, goods.specSku, shopCart)
    shopCart[index] = cart;
  }
...
function delShopCart(goods, cartTypeName) {
  ...
  if (cart.total - addTotal < 1) {
    ...
    for (let i = 0; i < shopCart.length; i++) {
      if (shopCart[i].goodsId == cart.goodsId && shopCart[i].specSku == cart.specSku) {
        index = i;
        break;
      }
    }
...
function setShopCartNum(goods, setTotal, cartTypeName, isGift) {
  ...
  let shopCart = getShopCart(cartTypeName)
  //获取购物车数据
  let cart = getShopCartGoods(goods, cartTypeName)
